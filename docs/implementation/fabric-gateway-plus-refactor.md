...
- Full **5-org** deployment per `network-manifest.json`
- JWT auth guards (module structure in place; guards deferred)
- `@nestjs/swagger` codegen (static OpenAPI retained)
- Fabric 3 **SmartBFT** orderer
- Web `VITE_API_BASE_URL` Docker networking fix
...
Split the former monolithic `pds.controller.ts` into domain controllers under `apps/api/src/modules/`; **URL paths are unchanged**.
...
**Resolved:** `PDS_LEDGER_MODE` is the top-level switch; legacy `PDS_LEDGER_BACKEND` maps via [`ledger-mode.config.ts`](apps/api/src/modules/config/ledger-mode.config.ts).
...
Keep [`FabricEnvelopeLedgerPort`](apps/api/src/modules/fabric/fabric-envelope-ledger-port.ts) and `local-file` as **test-only** adapters behind `PDS_LEDGER_BACKEND=local-file|fabric-envelope` for dev tooling.
...
- [`blockchain/fabric-network/README.md`](../../blockchain/fabric-network/README.md) — live stack and bootstrap
- [`README.md`](../../README.md) — `docker compose --profile fabric` quickstart
- [`docs/implementation/mvp-implementation-plan.md`](mvp-implementation-plan.md) — Gate 1/2 progress
...
See **Follow-ups** in the Implementation status section above.
