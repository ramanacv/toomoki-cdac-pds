...
| 2. Transactional commands | Blocked for release | Row-scoped commands and business event/outbox insert share one DB transaction; independent worker; concurrency/crash tests | Current **supply-chain** snapshot engine and full-state writes must be replaced. Beneficiary registry and eligibility checkpoint/final paths already use row-scoped atomic outbox inserts (see [mocks-integrity-proof-completeness-plan.md](mocks-integrity-proof-completeness-plan.md)). Initial worker claim/retry/dead-letter schema is present but the poller is still embedded. |
