...
This list is additive to the
[J&K/Maharashtra integration gates](jkmaha-epos-smartpds-implementation-plan.md)
and the [transactional-command hardening tracker](mvp-hardening-plan.md).
...
| P0 before pilot | Federation with approved IAM, organization-scoped Fabric identities, enrollment, revocation and HSM keys | Local Keycloak users, service clients, and generated Fabric identities are not pilot credentials. |
...
| P1 | Department/NIC-approved SMART-PDS/RCMS, IAeSCM/state-SCM, AePDS/ePoS, authentication, logistics, and reconciliation contracts | Canonical demo adapters are fixtures, not approved external system contracts. |
