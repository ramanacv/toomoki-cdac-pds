...
const lifecycle = async (index) => {
  ...
  const quantityKg = 25;
  const cardHash = `benchmark-ration-card-hash-${prefix.toLowerCase()}`;
  const beneficiaryHash = `benchmark-beneficiary-hash-${prefix.toLowerCase()}`;
...
await runPool(20, Number(process.env.PDS_BENCHMARK_LIFECYCLE_CONCURRENCY ?? '2'), lifecycle);
...
const result = {
  runId,
  environment: { platform: platform(), release: release(), cpuCount: cpus().length, memoryGiB: Number((totalmem() / 2 ** 30).toFixed(1)), node: process.version, channel: network.fabric?.channel, chaincode: network.fabric?.chaincode, mspId: network.fabric?.mspId },
  workload: { authenticatedReads: 600, readConcurrency: [1, 5, 10], lifecycleSequences: 20, lifecycleConcurrency: Number(process.env.PDS_BENCHMARK_LIFECYCLE_CONCURRENCY ?? '2') },
