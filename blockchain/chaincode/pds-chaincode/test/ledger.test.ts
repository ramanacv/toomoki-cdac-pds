...
describe('PdsLedgerEngine', () => {
  ...
  it('conserves stock across dispatch + receive for both orgs and rejects re-dispatch after RECEIVED (T1.3)', () => {
    ...
    const firstTransfer = engine.exportState().transfers.find((item) => item.transferId === 'TR-CONSERVE-1');
    expect(firstTransfer?.lotId).toBe('LOT-CONSERVE-SPLIT-TR-CONSERVE-1');
    const parentAfterSplit = engine.exportState().lots.find((item) => item.lotId === 'LOT-CONSERVE');
    ...
    // Remaining parent lot stays at the sender and can be dispatched next.
    engine.dispatchLot({
    ...
    // Parent is fully moved; further dispatch from PROC must be rejected.
    expect(() =>
      engine.dispatchLot({
    ...
    ).toThrow(/owned by|DISPATCHED|remaining quantity|Insufficient stock/);
  });
  ...
  it('rejects dispatch with non-positive or over-stock dispatchedQtyKg (T1.3)', () => {
    ...
    ).toThrow(/Insufficient stock|remaining quantity/);
  });
